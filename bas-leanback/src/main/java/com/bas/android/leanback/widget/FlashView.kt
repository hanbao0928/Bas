...
import com.bas.core.lang.avoidZeroDividend
...
class FlashView @JvmOverloads constructor(
    ...
    init {
        ...
        setImageResource(R.drawable.bas_flash_img)
    ...
    private fun getTranslationOffset(parentWidth: Int, parentHeight: Int): Float {
        ...
        if (offset <= 0) {
            ...
            if (drawableWidth <= 0 || drawableHeight <= 0) {
                val bgDrawable = ContextCompat.getDrawable(context,R.drawable.bas_flash_img)
                drawableWidth = bgDrawable?.intrinsicWidth.orDefault()
                drawableHeight = bgDrawable?.intrinsicHeight.orDefault()
            }

            if (drawableWidth <= 0 || drawableHeight <= 0) {
                return 60f
            } else {
                return drawableWidth.toFloat() / drawableHeight.avoidZeroDividend() * parentHeight
            }
        }
        return offset
    }
        ...
        return ValueAnimator.ofFloat((-offset).toFloat(), (parentWidth + offset).toFloat())
            .also {
                it.addUpdateListener {
                    ...
                    val alpha = aFloat / parentWidth.avoidZeroDividend()
