 ...
 */
fun PressedColorStateList(
    ctx: Context,
    @ColorRes defColorId: Int,
    @ColorRes pressColorId: Int
): ColorStateList {
    return ColorStateList(
        PressedState(),
        intArrayOf(
            ContextCompat.getColor(ctx, pressColorId),
            ContextCompat.getColor(ctx, defColorId)
        )
    )
}
 ...
 * @param pressColorId 按下状态颜色资源id
 */
fun PressedColorStateListDrawable(
    ctx: Context,
    @ColorRes defColorId: Int,
    @ColorRes pressColorId: Int
): Drawable {
    return PressedColorStateListDrawable(
        ContextCompat.getColor(ctx, defColorId),
        ContextCompat.getColor(ctx, pressColorId)
    )
}