...
abstract class BasePresenter<D, V : BaseViewHolder> : Presenter() {
    ...
    private val itemKeyListenerProvider : ItemKeyListenerProvider<D,V> = object :ItemKeyListenerProvider<D,V>{
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, item: Any?) {
        ...
        if (itemClickListener != null) {
            var clickHolder =
                holder.view.getTag(R.id.item_click_listener_tag_bas) as? ItemClickListenerHolder<D, V>
            if (clickHolder == null) {
                clickHolder = ItemClickListenerHolder(itemClickListenerProvider, holder, data)
                holder.view.setOnClickListener(clickHolder)
                holder.view.setTag(R.id.item_click_listener_tag_bas, clickHolder)
            }
        ...
        if (itemFocusChangedListener != null) {
            var listenerHolder =
                holder.view.getTag(R.id.item_focus_listener_tag_bas) as? ItemFocusChangedListenerHolder<D, V>
            if (listenerHolder == null) {
                ...
                holder.view.setTag(R.id.item_focus_listener_tag_bas, listenerHolder)
            }
        ...
        if(itemKeyListener != null){
            var listenerHolder =
                holder.view.getTag(R.id.item_key_listener_tag_bas) as? ItemKeyListenerHolder<D, V>
            if (listenerHolder == null) {
                listenerHolder =
                    ItemKeyListenerHolder(itemKeyListenerProvider, holder, data)
                holder.view.setOnKeyListener(listenerHolder)
                holder.view.setTag(R.id.item_key_listener_tag_bas, listenerHolder)
            }
            listenerHolder.data = data
        }
    ...
    private class ItemKeyListenerHolder<D, V : BaseViewHolder>(
        ...
        override fun onKey(v: View?, keyCode: Int, event: KeyEvent?): Boolean {
            return provider.getItemKeyListener()?.onItemKey(v, keyCode, event, holder, data)
                .orDefault(false)
        }
    }
