...
/**
 * Created by Lucio on 2020-02-22.
 * 代码转载自：https://gist.github.com/objcode/7ab4e7b1df8acd88696cb0ccecad16f7#file-concurrencyhelpers-kt-L91
 * 协程并发帮助类
 * 提供功能：
 *  [SingleRunner]：同一时间只进行一个任务（类似队列）
 *  [ControlledRunner]：
 *  [cancelPreviousThenRun]：取消前一个任务 ;ControlledRunner对象在不同地方调用此方法，最后调用的地方会取消之前调用的地方，从而只有最后调用方法的地方能够得到结果
 *
 *  [joinPreviousOrRun]:加入之前的任务；ControlledRunner对象在不同地方调用此方法，所有调用方法的地方都会收到同一个结果
 *
 */
...
class ControlledRunner<T> {
    ...
    @JvmOverloads
    fun cancelActiveTask(cause: CancellationException? = null){
        activeTask.get()?.cancel(cause)
    }

    @JvmOverloads
    suspend fun cancelAndJoinActiveTask(cause: CancellationException? = null){
        activeTask.get()?.cancelAndJoin()
    }
}